...
# Load workspace
load("04_STDCheckOut/04_STDCheckOut.RData")
...
taxa.wo.std2 <- cbind(taxa.wo.std2, tax.seq.length)
...
ps.trim <- prune_taxa(as.numeric(tax_table(ps)[,"tax.seq.length"]) > 245 &
...
b3 <- plot_richness(ps.rich100, x = "Method", measures = c("Observed")) + 
  ggtitle("DNA extraction test") + facet_wrap("Site") +
  ylim(0, 300) + ylab("No. of ASVs (>100 copies/ml water)")
...
OpenDev(5, 8)
plot_grid(b2, b3, ncol = 1, labels = "auto")
dev.off()

OpenDev(9, 4.2)
plot_grid(p1 + theme(legend.position = "none"),
          p2 + theme(legend.position = "none"),
          p.legend, rel_widths = c(1,1,0.3), nrow = 1, labels = c("a","b",NULL))
dev.off()

...
strs5 <- round(nmds.bray.all$stress, 4)
strs6 <- round(nmds.bray.all2$stress, 4)
r5 <- p5 + theme(legend.position = "none") + ggtitle("All ASVs") + 
  ...
  annotate("text", x = 0.6, y = 0.6, label = sprintf("Stress = %s", strs5))
s5 <- q5 + theme(legend.position = "none") + ggtitle("Repeatedly detected ASVs") + 
  ...
  annotate("text", x = 2, y = 2, label = sprintf("Stress = %s", strs6))

...
#### No.1.2 Sequence analysis by DADA2
#### 2018.6.22 Ushio
...
# To reproduce the results, fastq files should be downloaded from DDBJ DRA (Submission ID: DRAXXXXXX for RMR-122)
sequence_id <- "RMR-121"
...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, truncLen=c(215,160),
                     # Sequences are trimmed at the same length with the 1st Run (RMR-032)
...
seqs.out <- as.matrix(c(rbind(sprintf(">Taxa_seq2_%05d", 1:length(seqs)), seqs)), ncol=1)
write.table(seqs.out, "01_ProSeqDADA2_Out/ProkASV_2nd_seqs.fa", col.names = FALSE, row.names = FALSE, quote = FALSE)

# Save objects
saveRDS(seqtab, "01_ProSeqDADA2_Out/objects/seqtab_2nd.obj")
saveRDS(seqtab2, "01_ProSeqDADA2_Out/objects/seqtab2_2nd.obj")
saveRDS(seqtab.nochim, "01_ProSeqDADA2_Out/objects/seqtab_nochim_2nd.obj")
saveRDS(track, "01_ProSeqDADA2_Out/objects/track_2nd.obj")
saveRDS(seqs.out, "01_ProSeqDADA2_Out/objects/seqs_out_2nd.obj")

# Save workspace and session info
save.image("01_ProSeqDADA2_Out/01_ProSeqDADA2_2nd_seq_Out.RData")
writeLines(capture.output(sessionInfo()),
           sprintf("00_SessionInfo/01_SessionInfo_2nd_seq_%s.txt", substr(Sys.time(), 1, 10)))
